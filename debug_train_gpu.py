...
parser.add_argument('--stepvalues', default='70000,90000', type=str, help='iter number wher elearing rate to be dropped')
...
parser.add_argument('--data_root', default='/media/PublicFile/zjg/realtime/', help='Location of VOC root directory')
parser.add_argument('--save_root', default='/media/PublicFile/zjg/realtime/realtime-lstm/save', help='Location to save checkpoint models')
...
def main():
    ...
    net.extras.apply(weights_init_for_extra)
    ...
    train(args, net, optimizer, criterion)
...
def train(args, net, optimizer, criterion, scheduler = None):
    ...
    for iteration in range(args.max_iter + 1):
        ...
        if iteration % args.print_step == 0 and iteration>0:
            ...
            print_line = 'Iteration {:06d}/{:06d} loc-loss {:.3f}({:.3f}) cls-loss {:.3f}({:.3f}) ' \
