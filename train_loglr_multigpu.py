...
from ssd_multigpu import build_ssd
import numpy as np
import time, copy
...
def main():
    ...
    parser.add_argument('--data_root', default='/home/lilin/my_code/realtime/', help='Location of VOC root directory')
    parser.add_argument('--save_root', default='/home/lilin/my_code/realtime/realtime-lstm/saveucf24/',
    ...
    net = torch.nn.DataParallel(build_ssd(300, args.num_classes), device_ids=args.gpus)
        # net = build_ssd(300, args.num_classes)

    if args.Finetune_SSD is True:
        print ("load snapshot")
        pretrained_weights = "/home/lilin/my_code/realtime/ucf24/rgb-ssd300_ucf24_120000.pth"
        pretrained_dict = torch.load(pretrained_weights)
        model_dict = net.module.state_dict()  # 1. filter out unnecessary keys
        pretrained_dict_2 = {k: v for k, v in pretrained_dict.items() if k in model_dict } # 2. overwrite entries in the existing state dict
        pretrained_dict_2['extras.3.bias'] = pretrained_dict['extras.2.bias']
        pretrained_dict_2['extras.3.weight'] = pretrained_dict['extras.2.weight']
        pretrained_dict_2['extras.4.bias'] = pretrained_dict['extras.3.bias']
        pretrained_dict_2['extras.4.weight'] = pretrained_dict['extras.3.weight']
        pretrained_dict_2['extras.6.bias'] = pretrained_dict['extras.4.bias']
        pretrained_dict_2['extras.6.weight'] = pretrained_dict['extras.4.weight']
        pretrained_dict_2['extras.7.bias'] = pretrained_dict['extras.5.bias']
        pretrained_dict_2['extras.7.weight'] = pretrained_dict['extras.5.weight']
        pretrained_dict_2['extras.9.bias'] = pretrained_dict['extras.6.bias']
        pretrained_dict_2['extras.9.weight'] = pretrained_dict['extras.6.weight']
        pretrained_dict_2['extras.10.bias'] = pretrained_dict['extras.7.bias']
        pretrained_dict_2['extras.10.weight'] = pretrained_dict['extras.7.weight']
...
def train(train_data_loader, net, criterion, optimizer, scheduler, epoch):
    ...
    for iteration in range(0, args.epoch_size):
        # load train data
        ...
        # forward
        out = net(images, img_indexs, args.gpus)
        # backprop
        optimizer.zero_grad()
        ...
        if iteration % args.print_step == 0 and iteration > 0:
            ...
            print_line = 'Epoch {:02d}/{:02d} Iteration {:06d}/{:06d} loc-loss {:.3f}({:.3f}) cls-loss {:.3f}({:.3f}) ' \
                ...
                epoch, args.epochs, iteration, args.epoch_size, loc_losses.val, loc_losses.avg, cls_losses.val,
